...
load_columns = function(filename) {
    ...
    find_prefixed_var_index()

    remove_null(dataframe, "NULL_PREDICTORx")

    find_prefixed_var_index(dataframe, "IS_FACTORx")
    find_prefixed_var_index(dataframe, "NOT_FACTORx")

    column_names = colnames(dataframe)

    for (i in 1:length(column_names)) {
        column = dataframe[,column_names[[i]]]
        # discard any columns which have less than 2 categories
        num_unique(column) < 2
...
ratio_unique = function(column) {
    return(num_unique(column) / nrow(column))
}

is_integer = function(column) {
    # integers could be stored as numeric, so round them off and check equality
    return(sum(round(column) == column) == length(column))
}

is_string = function(column) {
    # integers could be stored as numeric, so round them off and check equality
    return(is.character(column))
}
