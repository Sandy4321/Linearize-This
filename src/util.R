...
DEFAULT_REQUIRES = list(
    ...
    "MAX_ROWS" = 2500,
    "MIN_FACTOR_COLS" = 1,
    "MIN_NONFACTOR_COLS" = 1,
    "MIN_ROWS_PER_FACTOR" = 2,
...
is_factorizable = function(dataframe, colname, requires=DEFAULT_REQUIRES) {
    if (is.null(colname) || is.na(colname)) {
        return(FALSE)
    }
    return(min_factor_rows(dataframe[,colname]) >= requires$MIN_ROWS_PER_FACTOR)
}

min_factor_rows = function(column) {
    col_as_factor = as.factor(column)
    factor_count = table(col_as_factor)
    return(min(factor_count))
}
...
is_valid_data = function(dataframe, requires=DEFAULT_REQUIRES) {
    ...
    # categorize columns based on number of samples per factor
    factor_count = 0
    nonfactor_count = 0
    for (i in 1:num_columns) {
        if (is_factorizable(dataframe, columns[i])) {
            factor_count = factor_count + 1
        }
        else {
            nonfactor_count = nonfactor_count + 1
        }
    }
    if (factor_count < requires$MIN_FACTOR_COLS) {
        logging_print("ERROR: not enough factor columns", factor_count)
        return(FALSE)
    }
    if (nonfactor_count < requires$MIN_NONFACTOR_COLS) {
        logging_print("ERROR: not enough nonfactor columns", nonfactor_count)
        return(FALSE)
    }
